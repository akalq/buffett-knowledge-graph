...
REFERENCE_TRANSLATIONS = [
    ...
    # B级股说明
    ("Each Class B share has an economic interest equal to 1/30th",
    ...
    # 溢价发行
    ("There is an offset, however, to the reduction in book value",
    ...
    # 账面价值
    ("I dwell on this rise in per-share book value",
    ...
    # 股东手册
    ("For many years, we have listed these principles",
    ...
    # 现实因素
    ("Our expectations, however, are tempered by two realities",
    ...
    # 捐赠与开支
    ("In the table, the donations made pursuant to our shareholder-designated",
    ...
    # 内在价值说明
    ("In last year's letter, with Berkshire shares selling at $36,000",
    ...
    # 长期股东
    ("Of course, the longer a shareholder holds his shares",
    ...
    # KBS收购
    ("The first acquisition was Kansas Bankers Surety",
    ...
    # 生日宴会
    ("You might be interested in the carefully-crafted and sophisticated acquisition strategy",
    ...
    # 来信和成交
    ("On February 12, I got the following letter from Roy",
    ...
    # 成交过程
    ("Bob took it from there, and on September 18",
    ...
    # Al的年龄
    ("Al may be 79, but he looks and acts",
    ...
    ("To begin with, float is money we hold but don't own",
     "首先，浮存金是一项我们持有但不属于我们的资金，在保险公司的营运中，浮存金的产生原因在于保险公司在真正支付损失理赔之前，一般会先向保户收取保费，而通常保险业者收取的保费不足以因应最后支付出去的相关损失与费用，于是保险公司便会发生承保损失，这就是浮存金的成本，而当浮存金成本长期而言低于从其它管道取得资金的成本时，保险公司就有存在的价值，不过保险事业取得浮存金的成本若远高于资金市场利率时，它就像是一只在陆地上笨重的信天翁。"),
    
    # 浮存金成本
    ("typically do not cover the losses and expenses it eventually must pay",
     "通常情况下，若保险公司收取的保费不足以覆盖最终的理赔及相关费用，就会产生承保损失——这便是浮存金的成本。"),
    
    # 浮存金表格说明
    ("As the numbers in the following table show, Berkshire's insurance business has been a huge winner",
     "不过，如下表所示，伯克希尔的保险业务可谓大获全胜。相对于总保费收入，我们的浮存金规模相当可观，而浮存金的成本取决于实际的承保损失或收益。"),
    
    # 浮存金成本表
    ("浮存金成本是由我们的承保亏损或盈利决定的",
     "浮存金成本是由我们的承保亏损或盈利决定的。在我们承保盈利的年份，如过去四年，我们的浮存金成本为负数。实际上，我们为持有资金而获得了报酬。实际上，若是把伯克希尔拆成两部分的话，这两栏数字将分别代表这两个部门的损益绩效。"),
    ...
    # 我强调这一点
    ("I emphasize this lugubrious point because I would not want you to panic",
    ...
    # 巨灾再保险业务
    ("In our super-cat operation, our customers are insurers that are exposed",
     "谈到巨灾再保险业务，我们的客户主要是一些想要降低本身必须承担盈余变动剧烈风险的保险公司，而我们贩卖的产品——当然一定要以合理的价格，将这些盈余变动的风险转移到本公司的账上，因为我们对于伯克希尔公司盈余剧烈的变动一点都不会介意，查理跟我宁可接受上下变动但平均可达15%的结果，也不要平稳的12%。（就像是我们知道公司的盈余每天、每周都会变动，那么我们又何必强求公司的盈余变化一定要跟地球环绕太阳轨道的时间一致呢？）我想如果伯克希尔的股东合伙人也能有这样的看法，那么我们执行业务时便能更得心应手，而这也是为什么我们要一再提出相同警告的原因。"),
    ...
    # 最后一项优势
    ("Our final competitive advantage is that we can provide dollar coverages",
    ...
    # 加州地震局
    ("In a thoughtful response, Chuck Quackenbush",
    ...
    # Ajit Jain
    ("What I can state with certainty, however, is that we have the best person",
    ...
    # GEICO概述
    ("When we moved to total ownership of GEICO early last year",
    ...
    # 薪酬计划
    ("Today, the bonuses received by dozens of top executives",
    ...
    # 9000名员工
    ("Every quarter, all 9,000 GEICO associates can see the results",
    ...
    # 其它保险业务
    ("Though they are, of course, smaller than GEICO",
    ...
    # 税务
    ("In 1961, President Kennedy said that we should ask not what our country can do for us",
     "1961年，肯尼迪总统曾说过一句名言。去年，我们决定践行这一理念。最终，我们向美国国库缴纳了8.6亿美元的所得税。"),
    
    # 税务视角
    ("Here's a little perspective on that figure",
     "这个数字究竟有多大？如果全美国能有2,000家像伯克希尔这样的纳税人，那么美国国库无需再征收其他任何所得税，1996年的预算就能实现收支平衡。"),
    
    # 税务理念
    ("Charlie and I believe that large tax payments by Berkshire are entirely fitting",
    ...
    ("The table that follows shows the main sources of Berkshire's reported earnings",
     "下表列出了伯克希尔报告收益的主要来源。在这个表述中，购买法会计调整不分配给所适用的特定业务，而是汇总单独列示。这种程序让你能够以如果我们没有收购这些企业，它们本来会被报告的方式来查看我们企业的收益。由于第65和66页讨论的原因，这种形式的表述在我们看来对投资者和管理者都更有用，而不是使用一般公认会计原则（GAAP），后者要求将收购溢价逐个业务冲销。当然，我们在表格中显示的总收益与审计财务报表中的GAAP总额相同。"),
    
    # 透视盈余
    ("Though infrequently berkshire reports earnings calculated by strict",
     "虽然伯克希尔很少按公认会计准则（GAAP）来严格计算报告盈余，但查理和我相信，有一些数字虽然不属于 GAAP 的范畴，却对评估伯克希尔的经营成果至关重要。我们将它们称为"透视"盈余，定义如下：首先，加上我们报告的营业利润；其次，加上被投资公司的保留利润（如果这些保留利润被分配给我们的话，它们需要缴纳的税款按14%的税率计算）。"),
    ...
    # 股票投资
    ("Our portfolio shows little change",
    ...
    # 注定成功
    ("Companies such as Coca-Cola and Gillette might well be labeled",
    ...
    # 高科技公司
    ("Obviously many companies in high-tech businesses",
    ...
    # 美国航空
    ("When Richard Branson was asked how to become a millionaire",
    ...
    # 分析错误
    ("I liked and admired Ed Colodny",
    ...
    # 面值恢复
    ("In any event, the prices of USAir's publicly-traded securities tell us",
    ...
    # 所罗门融资
    ("We wrote four checks to Salomon Brothers last year",
    ...
    # 基金泡沫
    ("I think it would have been quite easy for such trusts to have sold many billions",
    ...
    # 部分股东
    ("Some Berkshire shareholders choosing to exit might have found that outcome ideal",
    ...
    # 长期投资者
    ("Overall, we tried to make sure that the B stock would be purchased only by investors",
    ...
    # 投行费用
    ("In recent years, it has been written that Charlie and I are unhappy about all investment-banking fees",
    ...
    # NFM
    ("NFM's main store, located on a 75-acre site about a mile from Aksarben",
    ...
    # 波仙珠宝
    ("Borsheim's normally is closed on Sunday but will be open for shareholders",
    ...
    # Gorat's牛排馆
    ("My favorite steakhouse, Gorat's, was sold out last year on the weekend",
    ...
    # 棒球比赛
    ("The Omaha Royals and Indianapolis Indians will play baseball on Saturday evening",
     "在前一天5月3日，星期六晚上，Rosenblatt体育馆将会有一场奥玛哈皇家队对印第安纳波利斯印第安人队的比赛，一如往年轮到由我先发，每一年就投那么一球。"),
    
    # Rosenblatt特殊
    ("Though Rosenblatt is normal in appearance, it is anything but",
    ...
    def replace_func(match):
        ...
        if en_len > 30 and cn_len < en_len * 0.5:
            ...
            for ref_en, ref_cn in REFERENCE_TRANSLATIONS:
                ...
                # 简单匹配：检查英文开头是否相同（前50个字符）
                if en_stripped[:50].lower() == ref_en_clean[:50].lower() and ref_cn:
                    total_fixed += 1
