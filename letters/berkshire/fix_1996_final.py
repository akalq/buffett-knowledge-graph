...
output_path = BASE_DIR / "投资研究/知识库/巴菲特知识库-LearnBuffett版/letters/berkshire/1996.html"
...
TRANSLATIONS = {
    ...
    "Salomon could not have performed better in the handling of this unusual transaction. Its investment bankers understood perfectly what we were trying to achieve and tailored every aspect of the offering to meet these objectives. The firm would have made far more money - perhaps ten times as much - if our offering had been standard in its make-up. But the investment bankers involved made no attempt to tweak the specifics in that direction. Instead they came up with ideas that were counter to Salomon's financial interest but that made it much more certain Berkshire's goals would be reached. Terry Fitzgerald captained this effort, and we thank him for the job that he did.": 
        "在这场不常见的交易中，所罗门的表现好得不能再好了，身为我们的投资银行，他们充分了解我们想要达成的目标，从而量身订做，提供符合我们需要的服务，事实上若是按照一般的标准模式，所罗门应该可以赚进更多钱，有可能比现在多十倍以上，不过他们并没有刻意引导我们这样子去做，相反地有时他们还是提出一些对自己本身利益冲突，但却有助于伯克希尔达成目的的一些建议，感谢Terry这次为我们操刀所做的努力与贡献。",
    ...
    "In recent years, it has been written that Charlie and I are unhappy about all investment-banking fees. That's dead wrong. We have paid a great many fees over the last 30 years - beginning with the check we wrote to Charlie Heider upon our purchase of National Indemnity in 1967 - and we are delighted to make payments that are commensurate with performance. In the case of the 1996 transactions at Salomon Brothers, we more than got our money's worth.": 
        "近年来，媒体不断报导查理跟我对于支付投资银行所有的费用都很感冒，这样的报导完全不对，事实上过去三十年来，从1967年我们请Charlie Heider协助我们买下国家赔偿保险公司开始，我们就很乐于签支票给他们，只要他们所提供的服务与表现值得的话，而以1996年所罗门这次的案子来说，我们就觉得物超所值。",
...
def find_best_translation(en_text):
    """根据英文文本找到最佳翻译"""
    en_clean = re.sub(r'\s+', ' ', en_text).strip()
    
    # 精确匹配
    if en_clean in TRANSLATIONS:
        return TRANSLATIONS[en_clean]
    
    # 尝试开头匹配
    for key, value in TRANSLATIONS.items():
        key_clean = re.sub(r'\s+', ' ', key).strip()
        if en_clean[:100].lower() == key_clean[:100].lower():
            return value
...
def fix_translations():
    ...
    def replace_func(match):
        nonlocal total_fixed, total_blocks
        ...
        # 跳过纯分隔符块
        if en_stripped.replace('*', '').replace('-', '').replace('=', '').replace('.', '') == '':
            return match.group(0)
        
        # 跳过表格数据块
        if re.search(r'\d{4}.*\$', en_stripped[:200]) and len(en_stripped) < 500:
            return match.group(0)
        
        total_blocks += 1
        
        # 统计字符数
        en_len = count_en_chars(en_stripped)
        cn_len = count_chinese_chars(cn_content)
        
        # 如果中文不足英文的50%，尝试使用翻译
        if en_len > 30 and cn_len < en_len * 0.5:
            translation = find_best_translation(en_stripped)
            if translation:
                total_fixed += 1
                return match.group(1) + en_content + match.group(3) + translation + match.group(5)
        
        return match.group(0)
    
    html_content = pattern.sub(replace_func, html_content)
    
    print(f"处理了 {total_blocks} 个双语块，修复了 {total_fixed} 个不完整翻译")
    return html_content
